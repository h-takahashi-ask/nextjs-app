...
## コーディング規約（このプロジェクト固有）
- **src/ 配下のすべての関数は、戻り値を直接 return せず一度変数に代入してから return する**
  - 理由：デバッグ時にブレークポイントで戻り値を確認できるようにするため
  - 変数名はコンポーネント・関数の役割を表す意味のある名前にする
  - ページコンポーネント → `page`、レイアウト → `layout`、UI部品 → コンポーネント名の camelCase（例: `sideNav`, `logo`）、API レスポンス → `response` / `errorResponse`
